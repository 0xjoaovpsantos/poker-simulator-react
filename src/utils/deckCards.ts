import copas_a from '../assets/copas/copas_a.png';
import copas_2 from '../assets/copas/copas_2.png';
import copas_3 from '../assets/copas/copas_3.png';
import copas_4 from '../assets/copas/copas_4.png';
import copas_5 from '../assets/copas/copas_5.png';
import copas_6 from '../assets/copas/copas_6.png';
import copas_7 from '../assets/copas/copas_7.png';
import copas_8 from '../assets/copas/copas_8.png';
import copas_9 from '../assets/copas/copas_9.png';
import copas_10 from '../assets/copas/copas_10.png';
import copas_j from '../assets/copas/copas_j.png';
import copas_q from '../assets/copas/copas_q.png';
import copas_k from '../assets/copas/copas_k.png';

import espadas_a from '../assets/espadas/espadas_a.png';
import espadas_2 from '../assets/espadas/espadas_2.png';
import espadas_3 from '../assets/espadas/espadas_3.png';
import espadas_4 from '../assets/espadas/espadas_4.png';
import espadas_5 from '../assets/espadas/espadas_5.png';
import espadas_6 from '../assets/espadas/espadas_6.png';
import espadas_7 from '../assets/espadas/espadas_7.png';
import espadas_8 from '../assets/espadas/espadas_8.png';
import espadas_9 from '../assets/espadas/espadas_9.png';
import espadas_10 from '../assets/espadas/espadas_10.png';
import espadas_j from '../assets/espadas/espadas_j.png';
import espadas_q from '../assets/espadas/espadas_q.png';
import espadas_k from '../assets/espadas/espadas_k.png';

import ouros_a from '../assets/ouros/ouros_a.png';
import ouros_2 from '../assets/ouros/ouros_2.png';
import ouros_3 from '../assets/ouros/ouros_3.png';
import ouros_4 from '../assets/ouros/ouros_4.png';
import ouros_5 from '../assets/ouros/ouros_5.png';
import ouros_6 from '../assets/ouros/ouros_6.png';
import ouros_7 from '../assets/ouros/ouros_7.png';
import ouros_8 from '../assets/ouros/ouros_8.png';
import ouros_9 from '../assets/ouros/ouros_9.png';
import ouros_10 from '../assets/ouros/ouros_10.png';
import ouros_j from '../assets/ouros/ouros_j.png';
import ouros_q from '../assets/ouros/ouros_q.png';
import ouros_k from '../assets/ouros/ouros_k.png';

import paus_a from '../assets/paus/paus_a.png';
import paus_2 from '../assets/paus/paus_2.png';
import paus_3 from '../assets/paus/paus_3.png';
import paus_4 from '../assets/paus/paus_4.png';
import paus_5 from '../assets/paus/paus_5.png';
import paus_6 from '../assets/paus/paus_6.png';
import paus_7 from '../assets/paus/paus_7.png';
import paus_8 from '../assets/paus/paus_8.png';
import paus_9 from '../assets/paus/paus_9.png';
import paus_10 from '../assets/paus/paus_10.png';
import paus_j from '../assets/paus/paus_j.png';
import paus_q from '../assets/paus/paus_q.png';
import paus_k from '../assets/paus/paus_k.png';

export default [
  {
    suit: 'copas',
    cards: [
      {
        id: 0,
        icon: copas_a,
        description: 'Copas 14',
        value: 14,
      },
      {
        id: 1,
        icon: copas_2,
        description: 'Copas 2',
        value: 2,
      },
      {
        id: 2,
        icon: copas_3,
        description: 'Copas 3',
        value: 3,
      },
      {
        id: 3,
        icon: copas_4,
        description: 'Copas 4',
        value: 4,
      },
      {
        id: 4,
        icon: copas_5,
        description: 'Copas 5',
        value: 5,
      },
      {
        id: 5,
        icon: copas_6,
        description: 'Copas 6',
        value: 6,
      },
      {
        id: 6,
        icon: copas_7,
        description: 'Copas 7',
        value: 7,
      },
      {
        id: 7,
        icon: copas_8,
        description: 'Copas 8',
        value: 8,
      },
      {
        id: 8,
        icon: copas_9,
        description: 'Copas 9',
        value: 9,
      },
      {
        id: 9,
        icon: copas_10,
        description: 'Copas 10',
        value: 10,
      },
      {
        id: 10,
        icon: copas_j,
        description: 'Copas 11',
        value: 11,
      },
      {
        id: 11,
        icon: copas_q,
        description: 'Copas 12',
        value: 12,
      },
      {
        id: 12,
        icon: copas_k,
        description: 'Copas 13',
        value: 13,
      },
    ],
  },
  {
    suit: 'espadas',
    cards: [
      {
        id: 13,
        icon: espadas_a,
        description: 'Espadas 14',
        value: 14,
      },
      {
        id: 14,
        icon: espadas_2,
        description: 'Espadas 2',
        value: 2,
      },
      {
        id: 15,
        icon: espadas_3,
        description: 'Espadas 3',
        value: 3,
      },
      {
        id: 16,
        icon: espadas_4,
        description: 'Espadas 4',
        value: 4,
      },
      {
        id: 17,
        icon: espadas_5,
        description: 'Espadas 5',
        value: 5,
      },
      {
        id: 18,
        icon: espadas_6,
        description: 'Espadas 6',
        value: 6,
      },
      {
        id: 19,
        icon: espadas_7,
        description: 'Espadas 7',
        value: 7,
      },
      {
        id: 20,
        icon: espadas_8,
        description: 'Espadas 8',
        value: 8,
      },
      {
        id: 21,
        icon: espadas_9,
        description: 'Espadas 9',
        value: 9,
      },
      {
        id: 22,
        icon: espadas_10,
        description: 'Espadas 10',
        value: 10,
      },
      {
        id: 23,
        icon: espadas_j,
        description: 'Espadas 11',
        value: 11,
      },
      {
        id: 24,
        icon: espadas_q,
        description: 'Espadas 12',
        value: 12,
      },
      {
        id: 25,
        icon: espadas_k,
        description: 'Espadas 13',
        value: 13,
      },
    ],
  },
  {
    suit: 'ouros',
    cards: [
      {
        id: 26,
        icon: ouros_a,
        description: 'Ouros 14',
        value: 14,
      },
      {
        id: 27,
        icon: ouros_2,
        description: 'Ouros 2',
        value: 2,
      },
      {
        id: 28,
        icon: ouros_3,
        description: 'Ouros 3',
        value: 3,
      },
      {
        id: 29,
        icon: ouros_4,
        description: 'Ouros 4',
        value: 4,
      },
      {
        id: 30,
        icon: ouros_5,
        description: 'Ouros 5',
        value: 5,
      },
      {
        id: 31,
        icon: ouros_6,
        description: 'Ouros 6',
        value: 6,
      },
      {
        id: 32,
        icon: ouros_7,
        description: 'Ouros 7',
        value: 7,
      },
      {
        id: 33,
        icon: ouros_8,
        description: 'Ouros 8',
        value: 8,
      },
      {
        id: 34,
        icon: ouros_9,
        description: 'Ouros 9',
        value: 9,
      },
      {
        id: 35,
        icon: ouros_10,
        description: 'Ouros 10',
        value: 10,
      },
      {
        id: 36,
        icon: ouros_j,
        description: 'Ouros 11',
        value: 11,
      },
      {
        id: 37,
        icon: ouros_q,
        description: 'Ouros 12',
        value: 12,
      },
      {
        id: 38,
        icon: ouros_k,
        description: 'Ouros 13',
        value: 13,
      },
    ],
  },
  {
    suit: 'paus',
    cards: [
      {
        id: 39,
        icon: paus_a,
        description: 'Paus 14',
        value: 14,
      },
      {
        id: 40,
        icon: paus_2,
        description: 'Paus 2',
        value: 2,
      },
      {
        id: 41,
        icon: paus_3,
        description: 'Paus 3',
        value: 3,
      },
      {
        id: 42,
        icon: paus_4,
        description: 'Paus 4',
        value: 4,
      },
      {
        id: 43,
        icon: paus_5,
        description: 'Paus 5',
        value: 5,
      },
      {
        id: 44,
        icon: paus_6,
        description: 'Paus 6',
        value: 6,
      },
      {
        id: 45,
        icon: paus_7,
        description: 'Paus 7',
        value: 7,
      },
      {
        id: 46,
        icon: paus_8,
        description: 'Paus 8',
        value: 8,
      },
      {
        id: 47,
        icon: paus_9,
        description: 'Paus 9',
        value: 9,
      },
      {
        id: 48,
        icon: paus_10,
        description: 'Paus 10',
        value: 10,
      },
      {
        id: 49,
        icon: paus_j,
        description: 'Paus 11',
        value: 11,
      },
      {
        id: 50,
        icon: paus_q,
        description: 'Paus 12',
        value: 12,
      },
      {
        id: 51,
        icon: paus_k,
        description: 'Paus 13',
        value: 13,
      },
    ],
  },
];
